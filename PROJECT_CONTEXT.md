...
## 4. Power & pump switching (3.3 V relay)
...
Pump switching uses a **3.3 V relay module** driven straight from the ESP32 — the module's onboard driver transistor and flyback do the work, so GPIO25 sources almost no current and no level-shifter is needed. Firmware therefore:
- Drives the pump as a **plain digital output on GPIO25** (`pinMode(OUTPUT)` + `digitalWrite`). **Never PWM a relay** — the coil would chatter. Pump control is on/off only.
- Uses a `PUMP_ACTIVE_LOW` compile-time flag for the module's polarity. **This build's module is active-HIGH (GPIO HIGH = pump ON), so the flag defaults to `false`.** For the common opto-isolated boards that energize on LOW, set it `true`.
- Wiring: `GPIO25 → relay IN`, `3V3 → relay VCC`, `GND → relay GND`, and the pump's +6 V feed through the relay `COM`/`NO` contacts. A **1N4007 across the pump** (band → +6 V) is optional insurance against contact arcing.
