...
```
   ┌──────────────┐
   │   WET BAY    │ 161 tall            ┌──────────────────┐
   │  cup on a    │                     │     DRY BAY      │ 95 tall
   │  fixed       │                     │ perfboard on     │
   │  pedestal    │   ← partition →     │ rails, pump      │
...
The partition is full height and solid. The **sensor wires** cross it — the colour chip's
I²C pair, the three analog probes and the DS18B20 — through two ⌀16 glands at z = 75, plus
the feed tube at z = 68.
...
| File | What it is |
|---|---|
| `body.stl` | The tray. Both bays, the partition, the cup pedestal, the perfboard rails, and every cutout. The only big print |
...
| `platform.stl` | The disc the cup stands on. Bolts to the top of the pedestal |

(`riser.stl` is left over from the earlier load-cell design and is no longer used — the
platform now bolts straight onto the pedestal.)

Print `platform` at 6 perimeters and 60 % infill so the cup sits on a flat, rigid disc.
...
```
   ...
   131  ── milk surface (175 mL)
    ...
    77.5── platform bottom       platform.stl, 5 thick
     3   ── pedestal base        printed into the floor, 74.5 tall
     0   ── bench
```
...
|---|---|---|
| Platform → pedestal | 2 × **M4 × 25**, from **above**, down through the platform | Counterbored so the heads sit below the cup floor. 15 mm apart |
...
Six screws and one zip tie for the whole instrument.

## Installing it

1. **Platform first.** Bolt the `platform` disc onto the top of the pedestal printed into the
   wet floor — two M4 from above, heads counterbored below the cup floor. Platform top lands
   at **z = 82.5**.
...
2. **TCS34725** on the two M2 bosses on the wet face of the partition, at mid-liquid height,
...
5. **TFT** onto the four M2 bosses behind the front window. (There is no touch pad — the
   device is driven from the browser dashboard, so the old pad pocket is unused.)
...
Keep the probes centred in the cup, not touching its wall, so each one reads the milk rather
than the acrylic. Clip the probe leads to the lid and to the box.
...
2. **Lift the cup out and fill it at the sink**, to the weir. Overfilling is the point — the
   weir sets a consistent fill line, so the probes are always submerged to the same depth.
   Fill it outside the box and the overflow goes down the drain instead of onto the bay floor.
3. Cup back in, lid on. Capture from the dashboard.
4. Lid off, cup out, dump the milk, cup back, lid on.
5. Flush from the dashboard. It pumps distilled water over the probes and into the cup.
...
| `dry_lid` | flat |
| `platform` | flat, 6 perimeters, 60 % infill — a flat, rigid disc for the cup |

```bash
openscad -o body.stl -D 'part="body"' hardware/enclosure-simple.scad
```

Walls are **3.0 mm on the wet bay and 2.4 mm on the dry bay and lids**. The wet bay is
thicker because it holds a full cup of milk and gets rinsed; the dry side just covers
electronics. Outer faces stay flush — the dry bay is simply 1.2 mm roomier inside.

Not printed: the cup (acrylic tube **⌀70 ID / ⌀76 OD × 70 tall**, ⌀6 weir hole drilled 45.5 mm
above the inside of its floor), TCS34725, perfboard, pump, tube, fasteners.
...
- **Measure every probe's length, and read the reach check.** This is the most important check
  ...
  lifts the liquid toward the lid — but keep the cup rim under 155. Keep each probe centred in
  the cup rather than letting it hang on its cable against the wall, so it reads the milk.
...
- **The turbidity sensor hangs inverted, and that is deliberate.** Per its datasheet it is
  ...
  that way here, so it drops into a printed collar under the lid, arms down in the milk,
  **pins up and dry**, positioned so the optical gap sits 10 mm below the surface. `turb_drop`
  is the adjustment.
...
- **This build uses a 3.3 V relay** to switch the pump, driven straight from GPIO25 (see
  [`docs/wiring.md`](wiring.md)). It is active-HIGH, so `PUMP_ACTIVE_LOW` defaults to `false`
  in [`sensors.h`](../libs/AquaMilkSensors/src/sensors.h); flip it to `true` if your module
  is active-LOW. The enclosure doesn't care either way.
